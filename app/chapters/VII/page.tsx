...
export default function TimelinePage() {
    ...
    useEffect(() => {
        ...
        if (banUntil && parseInt(banUntil) > Date.now()) {
            setBanMessage(
                `The clock whispers... patience, mortal. You may try again at ${new Date(
                    parseInt(banUntil)
                ).toLocaleTimeString()}.`
            );
        }
    ...
    useEffect(() => {
        ...
        while (
            ...
            yearProgress[years[newIndex]]?.length >= timelineData[years[newIndex]].length
            ) {
            newIndex++;
        }
    ...
    const handleSubmit = () => {
        ...
        if (newCorrect.length === 0) {
            ...
            localStorage.setItem(BAN_KEY, banTime.toString());
            setBanMessage(
                `The temporal gates reject you. Wait until ${new Date(banTime).toLocaleTimeString()} before meddling with time again.`
            );
            setError(null);
            return;
        }
        ...
        // Move to next year only when all logs are found for the current year
    ...
    return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center p-4 text-white font-mono">
            {banMessage ? (
                ...
                <>
                    <div className="mb-2 text-center">Enter the logs for the year {year}:</div>
                    <input
                        type="text"
                        value={inputValue}
                    ...
                    <div className="w-full max-w-lg h-4 bg-gray-700">
                        <div
                            ...
                            style={{
                                width: `${(Object.values(yearProgress).reduce((sum, arr) => sum + arr.length, 0) /
                                    Object.values(timelineData).reduce((sum, arr) => sum + arr.length, 0)) *
                                100}%`
                            }}
                        />
                    </div>
